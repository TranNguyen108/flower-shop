...
if(isset($_POST['send_reply'])){
   if(!verify_csrf_token($_POST['csrf_token'] ?? '')){
      ...
      db_update("UPDATE `order_reviews` SET admin_reply = ?, admin_reply_at = NOW() WHERE id = ?", 
...
if(isset($_GET['delete'])){
   if(!verify_csrf_token($_GET['token'] ?? '')){
      ...
      db_delete("DELETE FROM `order_reviews` WHERE id = ?", [$delete_id]);
...
<section class="messages">
   ...
   <?php
      $select_reviews = mysqli_query($conn, "
         SELECT r.*, u.name as user_name, u.email as user_email, o.placed_on as order_date, o.total_products
         FROM `order_reviews` r
         LEFT JOIN `users` u ON r.user_id = u.id
         LEFT JOIN `orders` o ON r.order_id = o.id
         ORDER BY r.created_at DESC
      ") or die('Lỗi truy vấn đánh giá');
   ...
   <div class="box" data-rating="<?php echo $fetch_review['rating']; ?>" data-replied="<?php echo empty($fetch_review['admin_reply']) ? 'not_replied' : 'replied'; ?>">
      <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1.5rem;">
         <div>
            <p style="font-size: 1.8rem; font-weight: 600; color: var(--text-primary); margin-bottom: 0.5rem;">
               <i class="fas fa-user"></i> <?php echo $fetch_review['user_name']; ?>
            </p>
            <p style="font-size: 1.4rem; color: var(--text-secondary);">
               <i class="fas fa-envelope"></i> <?php echo $fetch_review['user_email']; ?>
            </p>
         </div>
         <a href="reviews.php?delete=<?php echo $fetch_review['id']; ?>" class="delete-btn" onclick="return confirm('Xóa đánh giá này?');" style="margin-top: 0;">
            <i class="fas fa-trash"></i>
         </a>
      </div>
      
      <p style="font-size: 1.5rem; color: var(--text-secondary); margin-bottom: 0.5rem;">
         <i class="fas fa-calendar"></i> Đơn hàng: #<?php echo $fetch_review['order_id']; ?> - <?php echo $fetch_review['order_date']; ?>
      </p>
      
      <p style="font-size: 1.4rem; color: var(--text-secondary); margin-bottom: 1rem;">
         <i class="fas fa-box"></i> <?php echo $fetch_review['total_products']; ?>
      </p>
      ...
      <p style="font-size: 1.6rem; color: var(--text-primary); line-height: 1.8; margin: 1.5rem 0; padding: 1.5rem; background: var(--white); border-radius: 8px; border-left: 3px solid var(--primary);">
         ...
         <?php echo nl2br($fetch_review['message']); ?>
         <i class="fas fa-quote-right" style="color: var(--primary);"></i>
      </p>
      
      <?php if(!empty($fetch_review['images'])): ?>
      <div class="review-images">
         <?php 
         $images = explode(',', $fetch_review['images']);
         foreach($images as $img): 
         ?>
            <img src="../../assets/uploads/products/reviews/<?php echo $img; ?>" onclick="window.open(this.src, '_blank')" alt="Review image">
         <?php endforeach; ?>
      </div>
      <?php endif; ?>
      
      <?php if(!empty($fetch_review['videos'])): ?>
      <div class="review-videos" style="margin: 1rem 0;">
         <?php 
         $videos = explode(',', $fetch_review['videos']);
      ...
      <div class="admin-reply-display">
         ...
         <p style="font-size: 1.5rem; line-height: 1.8; color: var(--text-primary);">
            <?php echo nl2br($fetch_review['admin_reply']); ?>
         </p>
         <p style="font-size: 1.3rem; color: var(--text-light); margin-top: 1rem;">
            <i class="fas fa-clock"></i> <?php echo date('d/m/Y H:i', strtotime($fetch_review['admin_reply_at'])); ?>
         </p>
      </div>
      <?php endif; ?>
